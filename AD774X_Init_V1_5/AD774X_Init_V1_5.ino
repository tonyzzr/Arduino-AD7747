...
const uint8_t IDLE = B00000000;       // 0x00 is "OR" mask for start IDLE mode
...
void setup()
{
  ...
  for (uint8_t i = 0; i < sizeof(CAP_INTERRUPT_PIN); i++)
  {
    ...
    else
    {
      ...
      if (i == 0){attachInterrupt(digitalPinToInterrupt(CAP_INTERRUPT_PIN[i]), ReadNewData_1, FALLING);}
      if (i == 1){attachInterrupt(digitalPinToInterrupt(CAP_INTERRUPT_PIN[i]), ReadNewData_2, FALLING);}
      if (i == 2){attachInterrupt(digitalPinToInterrupt(CAP_INTERRUPT_PIN[i]), ReadNewData_3, FALLING);}
      if (i == 3){ attachInterrupt(digitalPinToInterrupt(CAP_INTERRUPT_PIN[i]), ReadNewData_4, FALLING);}
    }
  }
  //Start Channel 0 cont mode
...
void loop()
{
 ...
 //generate output
  if (totalFlag_done == B00001111)//change this to B11111111 when all enabled
  {
